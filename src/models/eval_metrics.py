...
class PulseEval():
    ...
    def peak_detection(self, target, pred, peak_min_height):
        ...
        for i, (pred_peaks, target_peaks) in enumerate(zip(pred_peaks_list, target_peaks_list)):
            ...
            matched_peaks_list.append(matched_peaks)
            matched_distances_list.append(matched_distances)
        return target_peaks_list, matched_peaks_list, matched_distances_list
    
    def debug_plot(self, pred, target, target_peaks_list, matched_peaks_list, matched_distances_list, debug_fnames):
        fig_width = 4
        fig_height = 16
        ...
        for i in range(target.shape[0]):
            ...
            for pi, peak in enumerate(matched_peaks_list[i]):
                if peak is not None:
                    ...
                    axs[plt_x, plt_y].text(peak, pred[i, peak], str(matched_distances_list[i][pi]), fontsize=12, color='red')
    ...
    def peak_error_at_height(self, target, pred, peak_min_height: float = 0.5, debug_fnames=None):
        target_peaks_list, matched_peaks_list, matched_distances_list = self.peak_detection(target, pred, peak_min_height)
        if debug_fnames:
            self.debug_plot(pred, target, target_peaks_list, matched_peaks_list, matched_distances_list, debug_fnames)
